...
class ViewSignature:
    ...
    def _args_flatten_map(self, args: List[FuncParam]) -> Dict[str, Tuple[str, ...]]:
        ...
        for arg in args:
            if is_pydantic_model(arg.annotation):
                for name, path, is_union_descendant in self._model_flatten_map(arg.annotation, arg.alias):
    ...
    def _model_flatten_map(self, model: TModel, prefix: str, is_union_descendant: bool = False) -> Generator[tuple[str, str, bool], None, None]:
        field: FieldInfo
        if get_origin(model) is Annotated:
            model = get_args(model)[0]
        if get_origin(model) in UNION_TYPES:
            ...
            for attr, field in model.model_fields.items():
                ...
                if is_pydantic_model(field.annotation):
                    yield from self._model_flatten_map(field.annotation, name, is_union_descendant)  # type: ignore
                else:
            ...
            for attr in path[1:]:
                ...
                if get_origin(annotation_or_field) in UNION_TYPES:
                    for arg in get_args(annotation_or_field):
                        if arg is type(None):
                            continue  # Skip NoneType
                        annotation_or_field = _detect_collection_fields(arg, attr, path, result)
                else:
                    annotation_or_field = _detect_collection_fields(annotation_or_field, attr, path, result)
    return result
